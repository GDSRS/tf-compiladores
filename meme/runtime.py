...
def eval(x, env=None):
    ...
    if head == Symbol.DEFINE:
        variable, path_or_operation = args
        path_or_operation = eval(path_or_operation,env)
        if isinstance(path_or_operation,str): # é o caminho para uma imagem
            path_or_operation =str(os.getcwd() +'/'+ path_or_operation[1:-1])
            image = Image.open(path_or_operation)
            draw = ImageDraw.Draw(image)
            env[Symbol(variable)] = (image,draw)
        else: # é uma operação
            env[Symbol(variable)] = path_or_operation
        ...
        variable, name = args
        variable = eval(variable,env)
        name = eval(name,env)[1:-1]
        variable[0].save(os.getcwd()+"/"+name)

    elif head == Symbol.SETFONT:
        fontfamily_path, size = args
        fontfamily_path =str(os.getcwd() +'/'+ fontfamily_path[1:-1])
        env[Symbol('font')] = ImageFont.truetype(fontfamily_path,size=size)
#
